...
class BdrDevice extends Homey.Device {
    ...
    async get_device_information() {
        api_endpoint = this.capabilities.system?.deviceInformationUri;
        return await this.driver.async_get_request(api_endpoint, this.token);
    }

    async get_operating_mode() {
        api_endpoint = this.capabilities.system?.operatingModeUri;
        return await this.driver.async_get_request(api_endpoint, this.token);
    }
    async get_status() {
        api_endpoint = this.capabilities.centralHeatingZones?.statusUri;
        return await this.driver.async_get_request(api_endpoint, this.token);
    }

    async set_target_temperature(target_temp) {
        api_endpoint = this.capabilities.centralHeatingZones?.putSetpointManualUri;
        payload = { roomTemperatureSetpoint: target_temp };
    ...
    async set_override_temperature(target_temp, override_end) {
        api_endpoint = this.capabilities.centralHeatingZones?.putSetpointTemporaryOverrideUri;
        payload = { roomTemperatureSetpoint: target_temp, temporaryOverrideEnd: override_end };
        return await this.driver.async_put_request(api_endpoint, this.token, payload);
    }
    async set_schedule(schedule_program) {
        api_endpoint = this.capabilities.centralHeatingZones?.putSetpointScheduleUri;
        payload = { currentHeatingTimeProgram: schedule_program };
        return await this.driver.async_put_request(api_endpoint, this.token, payload);
    }
    ...
    async get_consumptions() {
        api_endpoint = this.capabilities.producers?.energyConsumptionUri;
        return await this.driver.async_get_request(api_endpoint, this.token);
    }
    async get_water_pressure() {
        api_endpoint = this.capabilities.system?.waterPressureUri;
        return await this.driver.async_get_request(api_endpoint, this.token);
    }
    async get_errors() {
        api_endpoint = this.capabilities.system?.errorStatusUri;
        return await this.driver.async_get_request(api_endpoint, this.token);
    }
    async get_flow_temperature() {
        api_endpoint = this.capabilities.system?.flowTemperatureUri;
        return await this.driver.async_get_request(api_endpoint, this.token);
    }
    ...
    async async_update_water_pressure() {
        ...
        if (water_pressure != null && water_pressure.waterPressure != null) {
            ...
            this.setCapabilityValue('measure_pressure', this.water_pressure_value).catch(this.error);
    ...
    async async_set_preset_mode(preset_mode) {
        ...
        // Set a manual temperature
            ...
            await this.set_target_temperature(this._attr_target_temperature);
        }
        else if (bdr_preset_mode == "mode") {
            await this.set_operating_mode(program);
        }
    }
