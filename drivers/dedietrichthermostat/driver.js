...
class DeDietrichDriver extends BdrDriver {
    ...
    async onPair(session) {
        ...
        session.setHandler("start_pair", async (data) => {
            ...
            await this.login(data.username, data.password, 'dedietrich');
            const token = await this.pair(data.username, data.password, data.pairingcode, 'dedietrich');
            let store = {
                token: token
            };
            ...
            return { name: "dedietrich_" + data.pairingcode, store: store, settings: settings, capabilities: capabilities, capabilitiesOptions: capabilitiesOptions, data: { id: token } };
        });
